...
class TrendingController extends Controller
{
    ...
    /**
     * Menampilkan form untuk menambahkan trending manual
     *
     * @return \Illuminate\View\View
     */
    ...
    /**
     * Menyimpan trending manual dari form input
     *
     * @param Request $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function storeManual(Request $request)
    {
        ...
        // Cek apakah trending dengan judul yang sama sudah ada
        $existingTrending = Trending::where('judul', $validated['judul'])
            ->where('media_sosial_id', $validated['media_sosial_id'])
            ->whereDate('tanggal', Carbon::parse($validated['tanggal'])->toDateString())
            ->first();

        if ($existingTrending) {
            return redirect()->route('trending.manual.create')
                ->with('error', 'Trending dengan judul tersebut sudah ada untuk platform dan tanggal yang sama!')
                ->withInput();
        }
        ...
        $trending = new Trending([
            ...
            'judul' => $validated['judul'],
            'url' => $validated['url'],
            'is_selected' => isset($validated['is_selected']) ? true : false,
        ]);

        // Jika trending baru dipilih, atur display_order sesuai media sosial
        if ($trending->is_selected) {
            $mediaSosial = MediaSosial::find($validated['media_sosial_id']);

            if ($mediaSosial->nama == 'Google') {
                // Ambil urutan terakhir Google
                $maxOrderGoogle = Trending::where('is_selected', true)
                    ->whereHas('mediaSosial', function($query) {
                        $query->where('nama', 'Google');
        ...
        // Redirect ke halaman yang sesuai berdasarkan is_selected
        if ($trending->is_selected) {
            $mediaSosial = MediaSosial::find($validated['media_sosial_id']);

            if ($mediaSosial->nama == 'Google') {
                return redirect()->route('trending.manageGoogleSelected')
                    ->with('success', 'Trending Google berhasil ditambahkan dan dipilih!');
            } else if ($mediaSosial->nama == 'X') {
                return redirect()->route('trending.manageXSelected')
                    ->with('success', 'Trending X berhasil ditambahkan dan dipilih!');
            }
        }

        return redirect()->route('trending.index')
            ->with('success', 'Trending berhasil ditambahkan!');
    }
